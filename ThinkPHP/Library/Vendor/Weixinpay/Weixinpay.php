...
class Weixinpay {
    ...
    public function unifiedOrder($order){
        ...
        $config=array(
            ...
            'nonce_str'=>'test',
            'spbill_create_ip'=>get_ip_address(),
        ...
        $data['sign']=$sign;
        $xml=$this->toXml($data);
//        echo to_json_string($data)."<br>";
//        echo htmlspecialchars_decode($xml)."<br>";
//        exit(htmlspecialchars($xml));
...
//        ajaxMsg(1,htmlspecialchars($response));
...
//            }
     ...
     */
    public function getParameters($total_fee, $body, $out_trade_no, $product_id,$openid,$pay_type){
        // 获取配置项
        $config=$this->config;
        ...
        if (!isset($_GET['code'])) {
            ...
            // 返回的url  济困捐款的 为 1
            if($pay_type == '1'){
                $redirect_uri=U('Mob/PartyServe/pays','','',true);
            }else{
                $redirect_uri=U('Weixin/Weixinpay/pay','','',true);
            }
