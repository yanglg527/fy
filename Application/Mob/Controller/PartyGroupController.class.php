...
class PartyGroupController extends BaseAuthController {
    ...
    public function index()
    {
       ...
       // $group['pm'] = $count[0]['count']?$count[0]['count']:1;
        ...
        $this->assign('taizhang1',
            D('TaizhangView')->where(array('Taizhang.organization_id'=>$group['id'],'type'=>2,'Taizhang.status'=>array('neq',1),'Taizhang.status'=>array('gt',-1)))->order(array('create_time'=>'desc','dz_count'=>'desc'))->limit(10)->select());
        // //点赞数多的台账
    ...
    public function zone()
    {
        ...
        $this->assign('taizhang',
            D('TaizhangView')->where(array('Taizhang.organization_id'=>$group['id'],'type'=>2,'Taizhang.status'=>2))->order('Taizhang.create_time desc')->limit(20)->select());

        $this->assign('norms',
            D('TaizhangNorms')->where(array('type'=>2))->order('id asc')->select());


        $this->setWebTitle($group['name']);
        $this->assign('group',$group);
        //党委
        $users = D('PartyOrganizationMembers')->where(array('organization_id'=>$group['id']))->select();
        //拿书记名字
        $sj = D('PartyOrganizationMembers')->where(array('organization_id'=>$group['id'],'status'=>1))->find();
        //去拿头像
        foreach ($users as &$item){
           $img =  D('User')->where(array('uid'=>$item['uid']))->field('headimgurl')->find();
           $item['headimgurl'] = $img['headimgurl'];
        }
        //按职务大小排序
        $users = dutyorder($users);
        $users = un_repeat($users);
