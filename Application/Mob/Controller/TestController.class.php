...
class TestController extends BaseController
{
    ...
    public function ajaxLoadData()
    {
        ...
        $list = D('Exam')->where(array('status' => 1, 'publish_time' => array('lt', $published_at)))->select();
        ...
        if ($ids) {
            ...
            foreach ($subjects as $index => $subject) {
                ...
                if ($subject['type_num'] == 'choice') { //如果是多选题
                    //  var_dump($subject['right_answer']);
                    $subject['right_answer_str'] =  trim($subject['right_answer'], '"');
...
(select sum(exam_paper.total_count) from exam_paper where exam_paper.uid = user.uid and exam_paper.exam_id = $id) AS total_count,
(select sum(exam_paper.right_count) from exam_paper where exam_paper.uid = user.uid and exam_paper.exam_id = $id) AS right_count
from user LEFT JOIN party_branch 
         ON party_branch.id=user.branch_id  
          where (user.status=1) order by right_count desc");
                ...
                if ($ids) {
                    ...
                    foreach ($subjects as $index => $subject) {
                        ...
                        if ($subject['type_num'] == 'choice') { //如果是多选题
                            ...
                            $subject['right_answer_indexs'] = $righr_answer_indexs;
                            //                            //返回正确答案键值对
                            $subject['right_array'] = $answer;
                            //解析答案内容
                            $subject['answer'] = json_decode($subject['content'], true);
                        } elseif ($subject['type_num'] == 'singleChoice') { //如果是单选题
                            ...
                            $subject['right_answer_str'] = $show;
                            $subject['right_answer_indexs'] = $righr_answer_indexs;
                            reset($righr_answer_indexs);
                            $first_key = key($righr_answer_indexs);
                            $subject['right_key'] = $first_key;
                            //解析答案内容
                            $subject['answer'] = json_decode($subject['content'], true);
                        } elseif ($subject['type_num'] == 'blank') { //如果是填空题
                            ...
                            foreach ($answer as $index2 => $value) {
                                ...
                                $showContent = str_replace("value=\"" . $value . "\"", "value=\"" . $ua . "\" disabled=\"true\"  ra=\"" . $value . "\" ", $showContent);
                            ...
                            if (is_array($subject['user_score'])) {
                                ...
                                //全部对才显示正确
                                if ($subject['user_score'][0] < $subject['score'] * count($subject['user_score'])) {
                                    $subject['user_score'][0] = 0;
                                }
                            }
                        } elseif ($subject['type_num'] == 'answer') { //如果是简答题
                            //解析html 转义字符
