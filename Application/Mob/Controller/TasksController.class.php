        ...
        if (false !== $int || !empty($monitor)) {
            ...
            if ($item) {
                ...
                foreach ($item['files'] as $k => &$v) {
                    ...
                    if (in_array($v['filetype'], $exts)) {
                        $url = urlencode($v['files_path']);
                        $item['files'][$k]['files_path'] = 'https://view.officeapps.live.com/op/view.aspx?src='.$_SERVER['HTTP_HOST'].$url;
                    }
                }
            }
    ...
    public function initData(){
        ...
        switch ($tasksType) {
            ...
            case '2': // 个人
                ...
                // 显示列表过滤器
                $this->assign('showSearchInput', true);
                break;
        }
