...
class CenterController extends BaseAuthController {
	...
	public function index() {
		...
		$taizhang = D('TaizhangView') -> where(array('Taizhang.uid' => $user['uid'], 'Taizhang.status' => 0)) -> order('Taizhang.create_time desc')->select();
		//var_dump($taizhang);
	...
	public function ajax_save_reward() {
        ...
        if(!$name || !$content || !$level_name){
            ajaxMsg(1, "填写内容不完善，请检查后提交");
        }
        if($reward_date> time()){
            ajaxMsg(1, "入党日期不能超过当前月份");
        }
...
$adm_id == 0?null:$adm_id;
	...
	public function ajax_save() {
        ...
        if(strtotime($activist_date)> time()){
            ajaxMsg(1, "入党日期不能超过当前月份");
        }
		// $birthday = I('birthday');
		//        ,'mobile'=>$mobile,'realname'=>$realname,
		$array = array('gender' => $gender,  'sign' => $sign,'realname'=>$realname,'mobile'=>$mobile);
		if ($activist_date) {
