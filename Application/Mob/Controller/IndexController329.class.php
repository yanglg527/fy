...
//        }
//      待办任务
        $badge = M('TasksExecutor')->where(array(
            'exe_uid'=>uid(),
    ...
    public function ajaxNeWIndexLoading(){
        ...
        // if ($type_name == 0) {
        //     $where["type_name"]=$type_name;
        //     $articles=D('Articles')->where($where)->order('published_at desc,id desc')->limit(10)->select();
        // }else{
...
//            var_dump($token);
    ...
    public  function ajaxSaveTaizhang(){
      ...
      // file_put_contents('./taizhang.txt',var_export($tztouser,true));
        ...
        $tz_id = D("Taizhang")->add(array("uid"=>$userId,"title"=>$title,"publish_time"
        =>$publishDate,"type"=>$type_id,"tag_id"=>$tag_id,"address"=>$address,'status'=>0,"party_name"=>$branch_name,"template_id"=>3,'publish_name'=>$publishName,'tz_content'=>$content,
            "organization_id"=>$user['organization_id'],"branch_id"=>$user['branch_id'],"add_uid"=>$user['uid'],"create_time"=>$create_time,"mission_id"=>$mission_id)); //,'compere'=>$compere
        if(!$tz_id){
            ajaxMsg(0,'信息上传失败');
        }
        //台账推给多个人
        if($more_people){
            ...
            foreach($tztouser as $v){
                ...
                if($touser){
                    $tz_id = D("Taizhang")->add(array("uid"=>$v,"title"=>$title,"publish_name"=>$touser['realname'],"publish_time"
