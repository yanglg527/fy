...
class IndexController extends BaseController
{
    ...
    {
        $this->check_wx_redirect('test_index');//判断是否重定向登录
        $user = user();
        $list = D("ExamRoleView")->where(array(
            "Exam.status"=>1,
            'Exam.end_time'=>array('gt',time()),
            'ExamRole.role_id'=>$user['role_id']))->group('ExamRole.exam_id')->order("Exam.create_time desc")->select();
    ...
    public function detail()
    {
        $id = I('id');
        $id = $id?$id:I('state');
        $this->check_wx_redirect('test_detail',$id);//判断是否重定向登录
        $exam = D('ExamView')->where("Exam.id=$id and Exam.status=1")->find();

        if($exam) {
            $user = user();

            $roles = D('ExamRoleView')->where(array('ExamRole.exam_id'=>$id))->select();
            $exam['role_limit'] = '';
            $size = sizeof($roles);
            $count = 0;
