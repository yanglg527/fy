...
class PartyMemberController extends BaseAuthController {
    ...
    public function index()
    {
        ...
        $this->assign('taizhang1',
            D('TaizhangView')->where(array('type'=>1,'Taizhang.status'=>array('gt',-1)))->order(array('create_time'=>'desc','dz_count'=>'desc'))->limit(20)->select());
...
//            D('TaizhangView')->where(array('type'=>1,'Taizhang.uid'=>$user['uid'],'type2'=>0,'Taizhang.status'=>array('gt',-1)))->order(array('dz_count'=>'desc','create_time'=>'desc'))->limit(3)->select());
        ...
        $this->assign('users',D('UserView')->where(array('status'=>1))->order('User.sort desc')->limit(20)->select());
    ...
    public function zone()
    {
        ...
        $this->assign('taizhang',
            D('TaizhangView')->where(array('type'=>1,'Taizhang.uid'=>$uid,'Taizhang.status'=>array('gt',-1)))->order('Taizhang.create_time desc')->select());
        $this->assign('norms',
