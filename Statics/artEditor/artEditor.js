...
$('#fileUploadA1').click(function(){
	$('#fileUploadInput1').click();
	return false;
})
...
$.fn.extend({
		...
		try{
			$(_this._opt.imgTar).on('change', function(e) {
                ...
                reader.onload = function (f) {
            		...
            		var img = '<img src="'+ f.target.result +'" style="width:100%;display: block" />';
	...
	upload: function(data) {
		...
		$.ajax({
			...
			cache: false,
			error:_this._opt.uploadError
		})
		.then(function(res) {
			var src = _this._opt.uploadSuccess(res);
			if(src) {
				var img = '<img src="'+ src +'" style="width:100%;display: block" />';
			    _this.insertImage(img);
			} else {
