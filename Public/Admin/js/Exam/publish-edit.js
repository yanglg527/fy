function editBlankInput(id){
    var sel = '';
    if(id){
        // subject-889[]
        sel = $('input[name="subject-'+id+'[]"]')
    }else{
        return false;
    }
    console.log(sel)
    ...
    var str = `<span class="blank">${blank_val}</span>`;
    $(this).after(str)
...
$('.exam_subject').on('click', '.btn-edit', function () {
    ...
    if (subject.type_num == 'text') {//获取编辑大标题的 html
        ...
        html = editSingleHtml
            .replace(/#id#/gm, subject.id)
            .replace(/#title#/gm, subject.title)
            .replace('#remark#', subject.remark)
        ...
        if(subject.type_num == 'singleChoice'){
            ...
            **隐藏解析****/
            ...
            $('#blank-insert-' + subject.id).click(function () {
                //var id = $(this).attr('data');
                ueBlank.focus();
                ueBlank.execCommand('inserthtml', '<input  style="border: 0;color:#999;border-bottom: 1px solid;padding-left:10px;margin: 0 10px;" class="input-blank form-control" value="" placeholder="请输入答案" name="subject-' + subject.id + '[]"/>')
            })
...
$('.exam_subject ').on('click', '.btn-edit-finish', function () {
    ...
    if (subject.type_num == 'text') {
        ...
        var remark = parent.find('.editor-subject2').find('textarea[name="remark"]').val();
